...
task :update do
  ...
  if open(filename).grep(/^lastmod: .*/).length > 0
    mdatetime = File.mtime(filename).strftime('%Y-%m-%d %H:%M:%S')
    text = File.read(filename).gsub(/^lastmod: .*/, "lastmod: #{mdatetime}")
    File.open(filename, "w").write(text)
    puts "Updated publish date to #{mdatetime}"
  else
    puts "File not updated -- no [lastmod: ] FrontMatter found."
  end
end # task update
